...
//		StringTokenizer st = new StringTokenizer(std01, ",");
	...
	private static Product getProduct(String pdtArr) {
		...
		return new Product(arr[0].trim(), arr[1].trim());
	}

	private static Sale getSale(String saleArr) {
		String[] arr = saleArr.split(",");
		int no = Integer.parseInt(arr[0].trim());
		String code = arr[1];
		int price = Integer.parseInt(arr[2].trim());
		int saleCnt = Integer.parseInt(arr[3].trim());
		int marginRate = Integer.parseInt(arr[4].trim());
